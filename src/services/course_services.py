...
class CourseServices:
    ...
    async def get_courses_by_owner(
        ...
        """
        courses = await CourseRepository.get_courses_by_owner(
    ...
    async def get_users_on_course(
        ...
        """
        users = await CourseRepository.get_users_on_course(
    ...
    async def create_course(
        ...
        if not await UserRepository.is_user_exists(
            login=course.owner, session=session
        ):
            raise UserNotFound
    ...
    async def add_user_to_course(
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, detail="Course or user does not exist"
            )
