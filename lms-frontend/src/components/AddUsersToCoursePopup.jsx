...
const AddUsersToCoursePopup = ({ courseId, onClose, onUsersAdded }) => {
    ...
    useEffect(() => {
        const fetchUsers = async () => {
            ...
            try {
                ...
                const allUsersResponse = await axios.get('http://backend-cnt/api/users_logins');
                setAllUsers(allUsersResponse.data);

                // 2. Fetch users already in the course
                const usersInCourseResponse = await axios.get(`http://backend-cnt/api/users_from_course/${courseId}`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await axios.post('http://backend-cnt/api/register_users/', payload);
